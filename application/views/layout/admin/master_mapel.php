                    ...
                    <div class="modal-body">
                        <form id="form_add">
                            <div class="form-group">
                                ...
                                <input type="text" placeholder="Masukkan Kode Mapel" id="data_kode_add" name="kode"
                                    value="" class="form-control" required="required">
                            </div>
                            <div class="form-group">
                                <label>Nama Mapel</label>
                                <input type="text" placeholder="Masukkan Nama Mapel" id="data_name_add" name="nama"
                                    value="" class="form-control" required="required">
                            </div>
                        </form>
                    </div>
...
<script>
    $(document).ready(function(){
		$('.dataTable').DataTable( {
			ajax: {
				url: "<?php echo base_url('master/master_mapel/view_all/')?>",
				type: 'POST',
			},
			columnDefs: [
			{   targets: 0, 
    ...
    function do_add(){
        if($("#form_add")[0].checkValidity()) {
            $.ajax({
				url: "<?php echo base_url('master/add_master_mapel')?>",
				type: 'POST',
    ...
    function do_edit(){
        if($("#form_edit")[0].checkValidity()) {
            $.ajax({
				url: "<?php echo base_url('master/edit_master_mapel')?>",
				type: 'POST',
    ...
    function do_delete(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?php echo base_url('master/delete_master_mapel')?>",
                    type: 'POST',
                    data: {id:id},
                    dataType: "json",
                    success: function(data){
    ...
    function do_view(id){
        $('#view').modal('show')
        $.ajax({
            url: "<?php echo base_url('master/master_mapel/view_one')?>",
            type: 'POST',
            data: {id:id},
            dataType: "json",
            success: function(data){
