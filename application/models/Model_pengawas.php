...
class Model_pengawas extends CI_Model
{
	...
	public function generate_pengawas($data)
    {
        ...
        $populasi =  $this->inisialisasi($data['jml_populasi'],$data['guru'],$data['ruang']);
        for($i =0;$i <= $data['jml_generasi'];$i++){
            ...
            $new_fitness_values = $this->mutasi_coba($new_generation, $data['mutasi'], $data['guru'],$data['ruang'],$data);
            ...
            // Jika sudah, kembalikan individu terbaik sebagai solusi terbaik
                ...
                $populasi = $this->reproduce($selected,$data['mutasi'],$data['crossover'],$data['guru'],$data['ruang'],$data);
                $fitness = $new_fitness_values;
            }
        }
        return $best_chromosome;
    }
    ...
    public function inisialisasi($jml_populasi,$guru,$ruangs){
        ...
        for ($i = 0; $i < $jml_populasi; $i++) {
            $schedule = array();
                for($j=0;$j<count($ruangs)*2;$j++){
                    $class = array(
                    0 => $ruangs[array_rand($ruangs)],
        ...
        for ($i = 0; $i < count($individual); $i++) {
            for ($j = 0; $j < count($ruang)*2; $j++) {
                ...
                if (rand(0, 1) < $mutation_rate) {
                   ...
                   $this->individu[$i][$j][0] = $ruang[array_rand($ruang)];
